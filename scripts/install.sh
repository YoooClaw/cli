...
#
# 选项：
#   --version <v>   指定版本（默认最新 release）
#   --dir <path>    安装目录（默认 ~/.local/bin，已在 PATH 则用之；否则提示）
#   --force         覆盖已存在二进制
#
# 行为：
#   - 检测 OS/Arch，下载 GitHub Release 中的 yoooclaw-<os>-<arch>
#   - 校验 sha256（从同 release 的 checksums.txt 取）
...
if [ -z "$VERSION" ]; then
  ...
  [ -z "$latest_tag" ] && err "无法解析最新 stable tag（API 限流？请用 --version 显式指定）"

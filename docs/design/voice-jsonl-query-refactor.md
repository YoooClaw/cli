...
- `voice_id` 不包含扩展名和目录；生产端不能省略它并要求查询端长期从 `audio_rel_path` 生成。
- `voice_id` 一旦写入不可改变、不可复用；数字 `id` 只用于生产端内部兼容。

CLI 对外输出统一使用字符串 `id`，不输出数据库数字主键。新记录直接使用
`voice_id`；为兼容字段上线前已经落盘的历史记录，若 `voice_id` 缺失但
`audio_rel_path` 存在，Reader 使用音频文件名 stem；两者同时缺失或路径无法生成
有效 stem 时跳过该条。例如新格式源数据：
...
6. 中间的非法 JSON、字段校验失败或无法确定稳定 ID 的记录跳过并计数，不让单条坏记录使整次查询失败。

...
func newVoiceCmd() *cobra.Command {
	...
	today.Flags().String("app", "", "按 voice apps 返回的标准 app_name 过滤")

	stats := &cobra.Command{
		Use:   "stats",
		Short: "查询语音输入 App 的权威每日用量 🟢",
		Args:  cobra.NoArgs,
		RunE:  run(voiceStats),
	}
	stats.Flags().String("from", "", "开始本地日期（含，YYYY-MM-DD）")
	stats.Flags().String("to", "", "结束本地日期（不含，YYYY-MM-DD）")

	storagePath := &cobra.Command{
		Use:   "storage-path",
		Short: "打印语音输入历史目录和数据库路径 🟢",
		Args:  cobra.NoArgs,
		RunE:  run(voiceStoragePath),
	}

	c.AddCommand(list, search, show, apps, latest, today, stats, storagePath)
	return c
}
...
func addVoiceHistoryFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String("app", "", "按 voice apps 返回的标准 app_name 过滤")
...
func voiceShow(ctx *clictx.Context, cmd *cobra.Command, args []string) (any, error) {
	...
	if err != nil || id <= 0 {
		return nil, errs.New(errs.CodeInvalidArgument, "语音输入历史 ID 必须是大于 0 的整数")
	}
	repository, err := voice.Open(cmd.Context(), ctx.Paths.Voice)
	if err != nil {
		return nil, err
	...
	defer repository.Close()
	item, err := repository.Show(cmd.Context(), id)
	if err != nil {
		return nil, err
	}
...
func voiceApps(ctx *clictx.Context, cmd *cobra.Command, _ []string) (any, error) {
	...
	for i := range apps {
		rows[i] = map[string]any{
			"app_name": apps[i].AppName, "history_count": apps[i].HistoryCount,
...
func voiceStats(ctx *clictx.Context, cmd *cobra.Command, _ []string) (any, error) {
	from, err := voice.ParseLocalDate(flagStr(cmd, "from"), "--from")
	if err != nil {
		return nil, err
	}
	to, err := voice.ParseLocalDate(flagStr(cmd, "to"), "--to")
	if err != nil {
		return nil, err
	}
	if from != "" && to != "" && from >= to {
	...
	defer repository.Close()
	days, total, err := repository.Stats(cmd.Context(), from, to)
	if err != nil {
		return nil, err
	}
	...
	for i := range days {
		rows[i] = map[string]any{
			"local_date": days[i].LocalDate, "successful_count": days[i].SuccessfulCount,
			"duration_ms": days[i].DurationMS, "char_count": days[i].CharCount,
			"updated_at": days[i].UpdatedAt,
		}
	}
	var fromValue, toValue any
	if from != "" {
	...
	return map[string]any{
		"ok": true, "scope": "all_successful_voice_inputs",
		"range": map[string]any{"from": fromValue, "to": toValue},
		"total": total, "daily": rows,
	}, nil
}
...
func voiceStoragePath(ctx *clictx.Context, _ *cobra.Command, _ []string) (any, error) {
	return map[string]any{
		...
		"database": filepath.Join(ctx.Paths.Voice, "voice.sqlite3"),
	}, nil
}
